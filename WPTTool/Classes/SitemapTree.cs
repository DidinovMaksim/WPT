...
namespace WPTTool.Classes
{
    ...
    public class SitemapTree
    {
        ...
        /*public void ParseAllNodes(CheckParse check = CheckParse.WholeTree, int minDepth = -1, int maxDepth = -1)
        {
            for (int i = 0; i < rootNodes.Count; i++)
            {
                switch (check)
                {
                    case CheckParse.WholeTree:
                        {
                            ParseSpecificNode(rootNodes[i]);
                            ParseChildNodes(rootNodes[i]);
                            break;
                        }
                    case CheckParse.From:
                        {
                            if (minDepth >= 0 && minDepth <= this.maxDepth)
                                ParseChildNodes(rootNodes[i]);
                            break;
                        }
                    case CheckParse.To:
                    ...
                    case CheckParse.FromTo:
                        {
                            if (minDepth >= 0 && maxDepth >= 0 && minDepth <= maxDepth)
                                ParseChildNodes(rootNodes[i]);
                            break;
                        }
                    case CheckParse.SpecificLevel:
                        {
                            if (minDepth >= 0 && minDepth <= this.maxDepth)
                                ParseChildNodes(rootNodes[i]);
                            break;
        ...
        bool CheckDepth(int curDepth, int minDepth, int maxDepth)
        {
            if (minDepth >= 0 && maxDepth >= 0 && minDepth <= maxDepth)
        ...
        void ParseChildNodes(Node node, Func<int[], bool> checkFunc = null, int minDepth = -1, int maxDepth = -1, int curDepth = 0)
        {
            bool check = checkFunc != null ? checkFunc(new int[] { curDepth, minDepth, maxDepth }) : true;//нужно ли - посмотреть скорость, как вариант заменить массив на класс
            if (!node.IsParsed && check)
            {
                Console.WriteLine("Parsing: " + node.FullUrl);
                string[] href = Parser.getAllHref(node.FullUrl, true);
                AddNodesFromStrings(href);
                node.IsParsed = true;
                CalcTotal();
            }
            //Thread[] th = new Thread[node.ChildNodes.Count];
            check = check = checkFunc != null ? checkFunc(new int[] { curDepth + 1, minDepth, maxDepth }) : true;
            if (check)
                for (int i = 0; i < node.ChildNodes.Count; i++)
                {
                    if (!node.ChildNodes[i].IsParsed)
                    {
                        //string[] href = Parser.getAllHref(node.FullUrl, true);
                        AddNodesFromStrings(Parser.getAllHref(node.FullUrl, true));
                        node.IsParsed = true;
                        CalcTotal();
                    }
                }
            ...
            for (int i = 0; i < node.ChildNodes.Count; i++)
                ParseChildNodes(node.ChildNodes[i], checkFunc, minDepth, maxDepth, curDepth + 1);
        }
        void ParseChildNodes(Node node)
        {
            ...
            {
                pagesParsing++;
                Node tmp = node.ChildNodes[i];
                ThreadPool.QueueUserWorkItem(new WaitCallback(delegate (object state)
                {
                    ParseSpecificNode(tmp);
                }));
            }
            for (int i = 0; i < node.ChildNodes.Count; i++)
                ParseChildNodes(node.ChildNodes[i]);
        }*/
